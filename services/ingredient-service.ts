...
export type AddIngredientFormValues = {
    company_id: string;
    jfDisplayName: string;
    name: string;
...
function parseDecimalLoose(value: string | number | undefined | null): number {
...
export function buildAddIngredientPayload(
    values: AddIngredientFormValues,
    ...
    const userId = String(profileObj.id ?? profileObj._id ?? "");
    const companyFromForm = values.company_id.trim();
    const companyId =
        companyFromForm ||
        String(profileObj.company?.id ?? profileObj.company?._id ?? "");
    const createdIso = values.created_date ? `${values.created_date}T12:00:00.000Z` : now;
    const priceRaw = parseDecimalLoose(values.price);
    const priceStr = values.price.trim() === "" || Number.isNaN(priceRaw) ? "" : String(priceRaw);

    return {
        uploadType: "manual",
        source: "internal",
        requestDate: now,
        jfDisplayName: values.jfDisplayName.trim(),
        name: values.name.trim(),
        nutritionalSummary: values.nutritionalSummary.trim(),
        imageUrl: "",
        alternativeNames: "",
        ...
        form: [],
        processes: [],
        storageCondition: values.storageCondition.trim(),
        seasonality: values.seasonality.trim(),
        ...
        isUpcycled:
            values.isUpcycled.trim().toLowerCase() === "true" || values.isUpcycled === "1",
        upcycledCertification: "",
        ingredientTags: [],
        physchemProperties: [],
        nutritionScore: 0,
        uploadedBy: userId,
        ...
        priceUnit: values.priceUnit.trim() || "kg",
        isAdditive: Boolean(values.isAdditive),
    };
}
...
class IngredientService {
    ...
    async addIngredient(payload: Record<string, unknown>): Promise<unknown> {
        try {
            const { data } = await api.post(ENDPOINTS.INGREDIENT.ADD_INGREDIENT, payload);
            return data?.data ?? data;
        } catch (e) {
            console.log("[ingredientService] addIngredient failed", e);
            handleApiError(e, "Add Ingredient");
            throw e;
        }
    }
