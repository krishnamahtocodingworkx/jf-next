...
export function normalizeProductListItem(
  ...
  const id = String(
    (raw._id as string | undefined) ?? (raw.id as string | undefined) ?? "",
  );
...
/** Maps Add Product form values into the `api/v1/product/create-product` request body. */
export function buildCreateProductPayload(
  ...
  const ingredientId = (row: Record<string, unknown>) =>
    String((row.ingredient as { id?: string } | undefined)?.id || "");
  const isCustomIngredient = (row: Record<string, unknown>) =>
    ingredientId(row).startsWith("custom:");
  const customIngredientName = (row: Record<string, unknown>) => {
    const id = ingredientId(row);
    const display = (row.ingredient as { jf_display_name?: string } | undefined)
      ?.jf_display_name;
    if (typeof display === "string" && display.trim()) return display.trim();
    return id.startsWith("custom:") ? id.slice("custom:".length) : "";
  };
  ...
  const toPercentage = (weight: number) =>
  ...
  const ingredients = rows
    .filter(
      (row) =>
        !isCustomIngredient(row) && isValidMongoObjectId(ingredientId(row)),
    )
    .map((row) => {
      const weight = Number(row.weight) || 0;
      return {
        ingredient: ingredientId(row),
        weight,
  ...
  const newIngredients = rows.filter(isCustomIngredient).map((row) => {
    const weight = Number(row.weight) || 0;
    return {
      ingredient: customIngredientName(row),
      weight,
      unit: String((row.unit as string | undefined) || "g"),
      percentage: toPercentage(weight),
    };
  ...
  const payload: Record<string, unknown> = {
    ...
    servingSize: String(values.servingSize ?? "").trim(),
    upcCode: String(values.upcCode || "").trim(),
    cost: Number(values.cost) || 0,
    retailCost: Number(values.retailCost) || 0,
    profitMargin: String(values.profitMargin ?? "0.0"),
  };
...
export function parseCategoryListRows(
  data: unknown,
): Record<string, unknown>[] {
...
export function extractCategoryBundle(
...
function catalogCategoryLabel(raw: Record<string, unknown>): string {
  ...
  const sub = catalogReadString(raw, [
    "subcategory",
    "sub_category",
    "product_subcategory",
    "type",
...
export function apiProductToCatalogRow(
  ...
  else if (catalogReadString(raw, ["trend_direction"]) === "down")
    trendPositive = false;
  else if (catalogReadString(raw, ["trend_direction"]) === "up")
    trendPositive = true;

  const tags = catalogParseTags(raw);
  const tagList = tags.length
    ? tags
    : CATALOG_TAG_POOL[hash % CATALOG_TAG_POOL.length];
...
export function matchesCatalogCategoryPill(
  name: string,
  ...
  switch (filterB) {
    ...
    case "powders":
      return (
        /powder|collagen|adaptogenic|mushroom/i.test(n) &&
        !/protein\s*crunch/i.test(n)
      );
    case "snacks":
